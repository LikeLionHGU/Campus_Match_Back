...
이 문서는 원본 CSV 명세서를 README 용도로 다시 정리한 버전입니다.  
가독성을 위해 다음과 같이 정리했습니다.

- 경로의 불필요한 줄바꿈, 중복 표기, `(1)` 표시는 제거
- 같은 API가 여러 페이지에서 재사용되는 경우 `사용 페이지`만 병합
- 요청/응답 예시는 **원본 의미를 유지**하면서 보기 좋게 배치
- 원본 문서에 남아 있는 비정형 표기(`GET`인데 필터 객체가 적혀 있는 경우, 스마트 따옴표, 일부 오타)는 임의 해석하지 않고 **참고사항**으로 유지

## 기본 정보

- **Base URL**: `https://{YOUR_DOMAIN}`
- **Content-Type**: `application/json`
- **인증 방식**: Access Token / Refresh Token 기반
- **원본 기준 엔드포인트 수**: 70개

## 문서 사용 시 참고

1. `GET` 요청에 필터 조건이 적혀 있는 항목은 원본 시트 기준 표기입니다.  
   실제 구현에서는 **query parameter**인지, **request body**인지 코드를 기준으로 최종 확인하는 것을 권장합니다.

2. 아래 예시는 README 가독성을 위한 정리본입니다.  
   최종 Swagger, Postman, 백엔드 컨트롤러와 한 번 더 대조해서 확정하는 것을 권장합니다.
